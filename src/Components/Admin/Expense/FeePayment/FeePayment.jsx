...
export default function FeePayment() {
  ...
  const feePayments = useSelector(
    (state) => state.feeManagement.feePayments
  );

  const status = useSelector((state) => state.feeManagement.status);
  const [selectedItem, setSelectedItem] = useState(null);
  const [updateDialogOpen, setUpdateDialogOpen] = useState(false);
  const [feeStructureDialoge, setFeeStructureDialoge] = useState(false);
  ...
  useEffect(() => {
    if (user) {
      dispatch(getFeePayments({ school_id, academic_year_id }));
    }
  }, [user]);

  const handleUpdateNotice = (id, body) => {
    dispatch(updateFeeStructure({ id, body }));
  };

  const handleDeleteNotice = (id) => {
    dispatch(deleteFeePayment(id));
  };

  if (status === "loading") {
    return (
      <>
        <Loader name="Fee Structures" />
      </>
    );
  }

  if (isEmptyObject(feePayments)) {
    return (
  ...
  if (
    feePayments.length > 0 && feePayments.filter(
      (feestructure) => feestructure.class_id === classShowId
    ).length === 0 &&
    classShowId !== null
  ) {
    return (
        ...
        <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mt-4 ">
          {feePayments &&
            feePayments
              .filter((feeStructure) => {
                if (classShowId) {
                  return feeStructure.class_id === classShowId;
                }
                return feeStructure;
              })
              .map((feeStructure) => (
                <div key={feeStructure?.id} className="">
                  <Dialog
                    open={feeStructureDialoge}
                    onOpenChange={setFeeStructureDialoge}
                  >
                    <DialogTrigger
                      onClick={() => setSelectedItem(feeStructure)}
                    ...
                    {selectedItem && selectedItem?.id === feeStructure?.id && (
                      <DialogContent
                        style={{
                            ...
                            >
                              ...
                              <div className="text-end">
                                <button
                                  ...
                                  onClick={() =>
                                    handleDeleteNotice(feeStructure?.id)
                                  }
                                >
                                  Delete
                                </button>
                            ...
                            >
                              ...
                              <div className="text-end">
                                <button
                                  ...
                                  onClick={() => {
                                    handleUpdateNotice(feeStructure?.id, {
